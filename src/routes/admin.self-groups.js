...
import { adminLogout, checkAdminUnlocked, listFaresAdmin, verifyAdminPassword, deleteFare, } from "@/lib/fares.functions";
import { listTickets } from "@/lib/tickets.functions";
import { listSelfGroupPassengers, listSelfGroupApplications, updateSelfGroupPassenger, } from "@/lib/self-groups.functions";
import { listBookingsAdmin } from "@/lib/agent-bookings.functions";
...
function Page() {
    ...
    return (<>
      ...
      {confirmDelete && (<DeleteModal confirmDelete={confirmDelete} setConfirmDelete={setConfirmDelete} deletePassword={deletePassword} setDeletePassword={setDeletePassword} deleteErr={deleteErr} setDeleteErr={setDeleteErr} busyDelete={busyDelete} doDelete={doDelete}/>)}
    </>);
}
function DeleteModal({ confirmDelete, setConfirmDelete, deletePassword, setDeletePassword, deleteErr, setDeleteErr, busyDelete, doDelete }) {
    return (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-navy/80 p-4 backdrop-blur-md">
      <div className="w-full max-w-md rounded-2xl bg-background p-6 shadow-2xl ring-1 ring-gold/30">
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <input type="password" value={deletePassword} onChange={(e) => setDeletePassword(e.target.value)} placeholder="Admin Password" className="w-full rounded-xl border border-border bg-card py-3 pl-10 pr-4 text-sm font-semibold focus:border-gold focus:ring-1 focus:ring-gold/30" autoFocus onKeyDown={(e) => e.key === "Enter" && doDelete()}/>
          </div>
          {deleteErr && (<div className="rounded-lg bg-destructive/10 px-3 py-2 text-center text-xs font-bold text-destructive ring-1 ring-destructive/20">
              {deleteErr}
            </div>)}
          <div className="flex gap-3">
            <button onClick={() => { setConfirmDelete(null); setDeletePassword(""); setDeleteErr(null); }} className="flex-1 rounded-xl border border-border bg-card py-3 text-sm font-black uppercase tracking-wider text-muted-foreground hover:bg-secondary">
              Cancel
            </button>
            <button onClick={doDelete} disabled={busyDelete || !deletePassword} className="flex-1 rounded-xl bg-destructive py-3 text-sm font-black uppercase tracking-wider text-white shadow-lg hover:opacity-90 disabled:opacity-50">
...
function Panel({ onConfirmDelete }) {
    const qc = useQueryClient();
    const router = useRouter();
    const logout = useServerFn(adminLogout);
    const { data: fares = [] } = useQuery({ queryKey: ["fares", "admin", "all"], queryFn: () => listFaresAdmin({ data: { includeDeleted: true } }) });
    ...
    function parseSeatsTotal(seats) {
        if (!seats)
            return 0;
    ...
    function ticketsForFare(f) {
        const key = fareKey(f);
        return tickets.filter((t) => t.group_type === "self" && (t.fare_id === f.id || (t.sector || "").toUpperCase().includes(key) || (t.sector || "").includes(f.id.slice(0, 8))));
    }
    function passengersForFare(f) {
    ...
    const appliedPrefills = useMemo(() => selfFares.map((f) => ({
    ...
    async function exportList(kind, list, fileName, title, meta) {
        ...
        if (meta) {
            ...
            doc.setFontSize(9);
            let ly = y + 66;
            for (const l of meta.flightLines) {
                doc.text(l.toUpperCase(), 44, ly);
                ly += 13;
            }
            if (meta.baggage) {
                doc.text(`BAGGAGE ${meta.baggage.toUpperCase()}`, 44, ly + 4);
            }
            // Right side stats
            const stats = [
    ...
    async function exportAs(kind) {
        setShowExport(false);
        await exportList(kind, passengers, `Self Group - All Passengers - ${fmtDateShort(new Date().toISOString().slice(0, 10))}`, "Self Group Passengers");
    }
    return (<div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {showExport && (<div className="absolute right-0 top-full z-50 mt-1 w-44 overflow-hidden rounded-md bg-white text-navy shadow-xl ring-1 ring-black/10">
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-6 space-y-4">
        ...
        ].map(([k, label]) => (<button key={k} onClick={() => {
                setTab(k);
                setSelected(new Set());
            }} className={`rounded-lg px-4 py-2 text-xs font-bold uppercase tracking-widest transition ${tab === k
        ...
        {tab === "dashboards" && splitFares.active.length === 0 && (<div className="rounded-xl bg-card p-8 text-center text-sm text-muted-foreground ring-1 ring-border">
            No active <b>Self Group</b> fares. Open <Link to="/admin" className="text-navy underline">Group Fares</Link> to add one.
          </div>)}

        {tab === "sold" && splitFares.soldOut.length === 0 && (<div className="rounded-xl bg-card p-8 text-center text-sm text-muted-foreground ring-1 ring-border">
            No <b>Sold Out</b> groups yet.
          </div>)}

        {(tab === "dashboards" || tab === "sold") && currentFares.length > 0 && (<div className="flex flex-col gap-6 lg:flex-row">
          {/* Group selector */}
            ...
            <div className="overflow-hidden rounded-xl bg-card ring-1 ring-border">
              <div className="flex items-center justify-between gap-2 bg-[#0b1024] px-3 py-2 text-white">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setSelected(new Set(currentFares.map((f) => f.id)))} className="rounded border border-white/20 px-2 py-0.5 text-[10px] font-semibold hover:bg-white/10">All</button>
              ...
              <ul className="max-h-[70vh] divide-y divide-border overflow-y-auto">
                ...
                const lines = (f.flight_details || "")
                    ...
                    .map((s) => s.trim())
                    .filter(Boolean);
                return (<li key={f.id} className={isSoldOut ? "opacity-60 grayscale-[0.5]" : ""}>
                      <label className="flex cursor-pointer items-start gap-2 px-3 py-2 hover:bg-secondary/50">
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          {lines.length > 0 && (<span className="mt-0.5 block space-y-0.5">
                              {lines.map((l, i) => (<span key={i} className="block text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{l}</span>))}
                            </span>)}
          ...
          <div className="min-w-0 flex-1 space-y-6">
            {visibleFares.length === 0 && (<div className="rounded-xl bg-card p-8 text-center text-sm text-muted-foreground ring-1 ring-border">
            ...
            {visibleFares.map((f) => {
                ...
                return (<FareDashboard key={f.id} fare={f} passengers={pax} total={total} sold={sold} available={available} pnrs={pnrs} tickets={fareTickets} ticketById={ticketById} bookingById={bookingById} onConfirmDelete={onConfirmDelete} onSave={async (id, patch) => { await update({ data: { id, ...patch } }); await refetch(); }} onExport={(kind) => exportList(kind, pax, fileName, `${f.origin.toUpperCase()} → ${f.destination.toUpperCase()} · ${f.airline}`, {
                        airline: f.airline,
            ...
            {(() => {
                ...
                for (const f of selfFares)
                    for (const p of passengersForFare(f))
                        matched.add(p.id);
                const unlinked = passengers.filter((p) => !matched.has(p.id));
                if (unlinked.length === 0)
                    return null;
                return (<section className="overflow-hidden rounded-xl bg-card ring-1 ring-border">
                  ...
                  <PassengersTable passengers={unlinked} ticketById={ticketById} bookingById={bookingById} onSave={async (id, patch) => { await update({ data: { id, ...patch } }); await refetch(); }}/>
                </section>);
            })()}
          </div>
        </div>)}
...
function FareDashboard({ fare, passengers, total, sold, available, pnrs, onSave, onExport, onConfirmDelete, tickets, ticketById, bookingById, }) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="flex items-start gap-5">
            ...
            <div>
              ...
              {flightLines.length > 0 && (<div className="mt-2 space-y-0.5">
                  {flightLines.map((l, i) => (<p key={i} className="text-[13px] font-semibold uppercase tracking-wider text-white/90">{l}</p>))}
                </div>)}
              {fare.baggage && (<p className="mt-3 text-[13px] font-bold uppercase tracking-wider text-white/80">
          ...
          <div className="flex flex-wrap items-stretch gap-3">
            ...
            <div className="flex gap-3">
              ...
              <div className="rounded-lg bg-white/5 px-4 py-3 text-center ring-1 ring-white/15 min-w-[120px]">
                ...
                <p className="font-serif text-lg font-black text-emerald-400">
                  {(() => {
            const saleSum = tickets.reduce((sum, t) => sum + (Number(t.sale) || 0), 0);
            ...
            const sale = tickets.reduce((sum, t) => sum + (Number(t.sale) || 0), 0);
            ...
            <div className="flex flex-col items-center gap-2 self-center">
              ...
              <div className="relative">
                ...
                {menu && (<div className="absolute right-0 top-full z-50 mt-1 w-48 overflow-hidden rounded-md bg-white text-navy shadow-xl ring-1 ring-black/10">
...
function Stat({ label, value, tone }) {
    ...
    return (<div className="rounded-md bg-white/10 px-3 py-2 text-center ring-1 ring-white/20 min-w-[86px]">
...
function PassengersTable({ passengers, onSave, ticketById, bookingById, }) {
    return (<>
    ...
    <div className="overflow-x-auto">
      ...
      <table className="w-full min-w-[820px] border-collapse text-xs">
        ...
        <tbody>
          {passengers.length === 0 && (<tr><td colSpan={9} className="p-6 text-center text-muted-foreground">No passengers yet. Add a Self-Group Ticket in Group Tickets and it will land here automatically.</td></tr>)}
          {passengers.map((p, idx) => (<PaxRow key={p.id} p={p} sr={idx + 1} onSave={onSave} ticketById={ticketById} bookingById={bookingById}/>))}
        </tbody>
      </table>
    </div>
    </>);
}
function PaxRow({ p, sr, onSave, ticketById, bookingById, }) {
    ...
    return (<tr className={`border-b border-border hover:bg-secondary/40 ${saving ? "opacity-70" : ""}`}>
      ...
      <td className="p-1">
        <div className="flex flex-wrap gap-1">
          ...
          {attachments.map((a, i) => (<a key={i} href={a.url || `https://jqanltwhgdmckrlltdnh.supabase.co/storage/v1/object/public/booking-attachments/${a.path}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded bg-navy/10 px-1.5 py-0.5 text-[10px] font-bold text-navy hover:bg-navy/20" title={a.name}>
