  ...
  return (
    <>
      ...
      >
        ...
        <td className={`sticky right-0 z-10 px-3 py-4 align-middle shadow-[-1px_0_0_var(--border)] group-hover:bg-bg-primary ${submittedFocus ? "bg-booking-amber-soft" : !action.done ? "bg-bg-accent-tint" : "bg-card"}`}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5">
            {b.status !== "confirmed" && tickets.length === 0 && (
              <Tooltip><TooltipTrigger asChild><Button size="icon" variant="secondary" disabled={busy} onClick={() => ticketInputRef.current?.click()} className="h-8 w-8" aria-label="Upload ticket"><Upload className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Upload ticket</TooltipContent></Tooltip>
            )}
            {b.status !== "confirmed" && (
              <Tooltip><TooltipTrigger asChild><Button size="icon" disabled={tickets.length === 0 || busy} onClick={() => onUpdateStatus(b.id, "confirmed")} className="h-8 w-8" aria-label="Confirm ticket"><CheckCircle2 className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Confirm ticket</TooltipContent></Tooltip>
            )}
            <Tooltip><TooltipTrigger asChild><Button variant="outline" size="icon" onClick={onEdit} aria-label="Edit booking" className="h-8 w-8"><Pencil className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Edit booking</TooltipContent></Tooltip>
            <Tooltip><TooltipTrigger asChild><Button variant="outline" size="icon" onClick={onDelete} aria-label="Delete booking" className="h-8 w-8 text-booking-rose"><Trash2 className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Delete booking</TooltipContent></Tooltip>
            {tickets.map((t, i) => (
              <Tooltip key={i}><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => onRemoveTicket(t.path)} className="h-8 w-8 text-booking-rose" aria-label={`Remove ticket${tickets.length > 1 ? " " + (i + 1) : ""}`}><Ticket className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent>Remove ticket {tickets.length > 1 ? i + 1 : ""}</TooltipContent></Tooltip>
            ))}
          </div>
        </td>
      </motion.tr>
