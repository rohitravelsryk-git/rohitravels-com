...
export function LatestUpdatesButton({ variant = "header", className = "" }: { variant?: Variant; className?: string }) {
  ...
  return (
    <Link
      to="/updates"
      className={`inline-flex items-center gap-1.5 rounded-full bg-[#EAB308] px-4 py-2 text-[11px] font-black uppercase tracking-widest text-navy shadow-lg transition-all hover:scale-105 hover:brightness-110 active:scale-95 animate-pulse hover:animate-none ${className}`}
