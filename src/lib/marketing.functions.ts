...
export const generateMarketingCopy = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketingCopy> => {
    ...
    const system = [
      ...
      "",
      "broadcast = personal 1-to-1 WhatsApp broadcast (max 10 lines). MUST use the EXACT same format as `status` above (flag, origin/destination, airline, legs, baggage, fare, book link), but can add 1-2 friendly intro lines and must include the footer.",
      "community = community/group announcement (max 12 lines). MUST use the EXACT same format as `status` above (flag, origin/destination, airline, legs, baggage, fare, book link), but formatted as a high-impact WhatsApp Status/announcement with the footer.",
