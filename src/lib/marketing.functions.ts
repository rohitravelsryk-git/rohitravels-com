...
export const generateMarketingCopy = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketingCopy> => {
    ...
    const system = [
      ...
      "Airline Name",
      "",
      "<DD MMM ORG DST DEPTIME ARRTIME> ORIGIN DESTIBNATION (DIRECT)   ← one line per flight leg, suffix based on direct/connecting status",
      "",
      "Baggage: 20+10 KG",
      "",
      ...
      "Airline Name",
      "",
      "<DD MMM ORG DST DEPTIME ARRTIME> ORIGIN DESTIBNATION (DIRECT)   ← one line per flight leg",
      "",
      "Baggage: 20+10 KG",
      "",
