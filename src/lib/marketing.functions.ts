...
export const generateMarketingCopy = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketingCopy> => {
    ...
    const system = [
      ...
      "",
      "broadcast = personal 1-to-1 WhatsApp broadcast (max 10 lines). MUST use the EXACT same format as `status` above (flag, origin/destination, airline, legs, baggage, fare), but uses the BOOK_LINK and the footer.",
      "community = community/group announcement (max 12 lines). MUST use the EXACT same format as `status` above (flag, origin/destination, airline, legs, baggage, fare), but uses the BOOK_LINK and the footer.",
