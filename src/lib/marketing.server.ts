...
/** Multi-turn variant used by the site assistant; same gateway, same failure wording. */
export async function chatMessages(messages: ChatMessage[], model = CHAT_MODEL): Promise<string> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403)
      throw new Error(
        "AI access is not enabled for this workspace — turn on the AI connector in Lovable.",
      );
    if (res.status === 429)
